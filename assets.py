...
class Switch():
    def __init__(self, screen, rect, color):
        self.screen = screen
        self.rect = rect
        self.rect.h = self.rect.w // 1.64
        self.color = color
        self.circle_radius = self.rect.h//2 - 1
        self.x_pos = self.rect.x + self.rect.width - self.circle_radius - 1
    
    def toggle(self, darkmode):
        if darkmode:
            color = self.color
            if self.x_pos != self.rect.x + self.rect.width - self.circle_radius - 1:
                self.x_pos += 1
        else:
            color = (221, 221, 221)
            if self.x_pos != self.rect.x + self.circle_radius + 1:
                self.x_pos -= 1
        ...
        pygame.gfxdraw.aacircle(self.screen, self.x_pos, self.rect.centery, self.circle_radius, (255, 255, 255))
        pygame.gfxdraw.filled_circle(self.screen, self.x_pos, self.rect.centery, self.circle_radius, (255, 255, 255))
